...
export const WhtService = {
    ...
    calculate(params: {
        ...
        const gross = new Decimal(params.amount.toString());
        const rate = new Decimal(params.rate.toString());

        let whtAmount = new Decimal(0);

        if (params.isGrossUp) {
            // สูตร Gross-up: amount * (rate / (100 - rate))
            whtAmount = gross.mul(rate.div(new Decimal(100).sub(rate)));
        } else {
            // สูตรปกติ: amount * (rate / 100)
            whtAmount = gross.mul(rate.div(100));
        }

        // ปัดเศษทศนิยม 2 ตำแหน่งตามมาตรฐานบัญชีไทย
        whtAmount = whtAmount.toDecimalPlaces(2);
        const netPaid = gross.sub(whtAmount);
