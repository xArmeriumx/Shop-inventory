...
export async function getReportData(startDate?: string, endDate?: string): Promise<ReportData> {
  ...
  try {
    return await ReportService.getReportData(startDate, endDate, { userId: ctx.userId, shopId: ctx.shopId });
  } catch (error: any) {
...
export async function getTopProducts(startDate?: string, endDate?: string, limit = 10) {
  const ctx = await requirePermission('REPORT_VIEW_SALES');
  return ReportService.getTopProducts(startDate, endDate, limit, { userId: ctx.userId, shopId: ctx.shopId });
}

export async function getProfitByProduct(startDate?: string, endDate?: string) {
  const ctx = await requirePermission('REPORT_VIEW_SALES');
  return ReportService.getProfitByProduct(startDate, endDate, { userId: ctx.userId, shopId: ctx.shopId });
}
...
export async function getComparisonReport(
  ...
  const ctx = await requirePermission('REPORT_VIEW_SALES');
  return ReportService.getComparisonReport(period1Start, period1End, period2Start, period2End, { userId: ctx.userId, shopId: ctx.shopId });
}

export async function getStockValueReport() {
  const ctx = await requirePermission('PRODUCT_VIEW');
  return ReportService.getStockValueReport({ userId: ctx.userId, shopId: ctx.shopId });
}

export async function getInventoryTurnover(startDate?: string, endDate?: string) {
  const ctx = await requirePermission('REPORT_VIEW_SALES');
  return ReportService.getInventoryTurnover(startDate, endDate, { userId: ctx.userId, shopId: ctx.shopId });
}

export async function getSalesByCategory(startDate?: string, endDate?: string) {
  const ctx = await requirePermission('REPORT_VIEW_SALES');
  return ReportService.getSalesByCategory(startDate, endDate, { userId: ctx.userId, shopId: ctx.shopId });
}

export async function getProfitLossReport(startDate?: string, endDate?: string) {
  const ctx = await requirePermission('REPORT_VIEW_SALES');
  return ReportService.getProfitLossReport(startDate, endDate, { userId: ctx.userId, shopId: ctx.shopId });
}
...
export async function getExpenseByCategoryReport(startDate?: string, endDate?: string) {
  ...
  return ReportService.getExpenseByCategoryReport(startDate, endDate, { userId: ctx.userId, shopId: ctx.shopId });
}

export async function getSalesChannelReport(startDate?: string, endDate?: string) {
  const ctx = await requirePermission('REPORT_VIEW_SALES');
  return ReportService.getSalesChannelReport(startDate, endDate, { userId: ctx.userId, shopId: ctx.shopId });
}
...
export async function getCustomerRankingReport(
  ...
  return ReportService.getCustomerRankingReport(startDate, endDate, limit, { userId: ctx.userId, shopId: ctx.shopId });
}